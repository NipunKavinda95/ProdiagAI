...
if check:
    ...
    with open("models/scaler.pkl", "rb") as f:
        scaler_shap = pickle.load(f)
    with open("models/type_enc.pkl", "rb") as f:
        le_type_shap = pickle.load(f)
    with open("models/id_enc.pkl", "rb") as f:
        le_id_shap = pickle.load(f)
    ...
    sample = pd.DataFrame([{
        ...
        "sound_db":         sound,
        "machine_type_enc": le_type_shap.transform([machine_type])[0],
        "machine_id_enc":   le_id_shap.transform([machine_id])[0]
    }])

    sample_scaled  = scaler_shap.transform(sample)
    ...
    st.markdown(
        "Enter your machine sensor readings in the sidebar and click **Analyse** "
        "to get an instant health status prediction and AI maintenance recommendation."
    )
    st.markdown("""
    **How it works:**
